...
cd ~
git clone https://github.com/alloyproject/alloy-pool.git nodejs-pool
cd /usr/src/gtest
sudo cmake .
sudo make
...
nvm install v8.9.3
cd ~/nodejs-pool
sed -r "s/(\"bind_ip\": ).*/\1\"$IPADDRESS\",/" config_example.json > config.json
sed -i "s/\"pool_id\": 0,/\"pool_id\": 1,/g" config.test.json
npm install
npm install -g pm2
