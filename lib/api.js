...
app.get('/pool/stats', function (req, res) {
    ...
    var hashHistory = global.database.getCache('global_stats')['hashHistory'];

    var cachedHashes = 0;
    hashHistory.forEach(function(element) {
        cachedHashes += element.hs;
    });
    if (hashHistory.length) {
        localCache.hashRateAvg = Math.floor(cachedHashes / hashHistory.length);
    }
