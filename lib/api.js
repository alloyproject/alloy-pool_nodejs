...
function getAllWorkerStats(address, callback){
    ...
    if (globalStatsCache === false) { // for compatibility
        globalStatsCache = global.database.getCache("history:" + address);
        if (globalStatsCache === false) globalStatsCache = globalCache;
    }
    let returnData = {global: {
        lts: Math.floor(globalStatsCache.lastHash / 1000),
        identifer: 'global',
        hash: globalStatsCache.hash,
    ...
    identifiers.sort().forEach(function(identifier){
        ...
        if (cachedStatsData === false) { // for compatibility
            cachedStatsData = global.database.getCache("history:" + id2); 
            if (cachedStatsData === false) cachedStatsData = cachedData;
        }
...
function getAddressStats(address, extCallback){
    ...
    if (cachedStatsData === false) { // for compatibility
        cachedStatsData = global.database.getCache("history:" + address);
        if (cachedStatsData === false) cachedStatsData = cachedData;
    }
    let paidQuery = "SELECT SUM(amount) as amt FROM payments WHERE payment_address = ? AND payment_id = ?";
    let paidQuery24 = "SELECT SUM(amount) as amt FROM payments WHERE payment_address = ? AND payment_id = ? AND paid_time >= CURRENT_TIMESTAMP() - INTERVAL 24 HOUR";
    ...
    if (typeof(payment_id) === 'undefined') {
        ...
        paidQuery24 = "SELECT SUM(amount) as amt FROM payments WHERE payment_address = ? AND payment_id IS ? AND paid_time >= CURRENT_TIMESTAMP() - INTERVAL 24 HOUR";
    ...
    async.waterfall([
        function (callback) {
            ...
            return callback(null, {hash: cachedStatsData.hash, identifier: 'global', lastHash: Math.floor(cachedStatsData.lastHash / 1000),
        ...
        function (returnData, callback) {
            debug(threadName + "Checking MySQL total amount paid for /miner/address/stats");
            global.mysql.query(paidQuery24, [address_pt, payment_id]).then(function (rows) {
                if (typeof(rows[0]) === 'undefined') {
                    returnData.amtPaid24 = 0;
                } else {
                    returnData.amtPaid24 = rows[0].amt;
                    if (returnData.amtPaid24 === null) {
                        returnData.amtPaid24 = 0;
                    }
                }
                return callback(null, returnData);
...
app.get('/pool/altblocks/:pool_type', function (req, res) {
    let limit = typeof(req.query.limit) !== 'undefined' ? Number(req.query.limit) : 25;
    let page = typeof(req.query.page) !== 'undefined' ? Number(req.query.page) : 0;
...
app.get('/pool/altblocks', function (req, res) {
    let limit = typeof(req.query.limit) !== 'undefined' ? Number(req.query.limit) : 25;
    let page = typeof(req.query.page) !== 'undefined' ? Number(req.query.page) : 0;
    res.json(global.database.getAltBlockList().slice(page*limit, (page + 1) * limit));
});

app.get('/pool/coin_altblocks/:coin_port', function (req, res) {
    let limit = typeof(req.query.limit) !== 'undefined' ? Number(req.query.limit) : 25;
    let page = typeof(req.query.page) !== 'undefined' ? Number(req.query.page) : 0;
...
app.get('/miner/:address/stats/:identifier', function (req, res) {
    ...
    if (cachedStatsData === false) { // for compatibility
        cachedStatsData = global.database.getCache("history:" + memcKey);
        if (cachedStatsData === false) cachedStatsData = cachedData;
    }
    return res.json({
        lts: Math.floor(cachedStatsData.lastHash / 1000),
...
secureRoutes.get('/', function (req, res) {
    global.mysql.query("SELECT payout_threshold, email, enable_email, enable_payout, telegram_id, enable_tlg_payout, enable_tlg FROM users WHERE id = ?", [req.decoded.id]).then(function(row){
        return res.json({msg: {payout_threshold: row[0].payout_threshold, email: row[0].email, email_enabled: row[0].enable_email, enable_payout: row[0].enable_payout,  telegram_id: row[0].telegram_id, enable_tlg_payout: row[0].enable_tlg_payout, enable_tlg: row[0].enable_tlg}});
    });
});
...
secureRoutes.post('/changeTlg', function (req, res) {
    global.mysql.query("UPDATE users SET telegram_id = ? WHERE id = ?", [req.body.telegram_id, req.decoded.id]).then(function () {
        return res.json({'msg': 'Updated telegram_id was set to: ' + req.body.telegram_id});
    });
});
...
secureRoutes.post('/togglePayout', function (req, res) {
    global.mysql.query("UPDATE users SET enable_payout = NOT enable_payout WHERE id = ?", [req.decoded.id]).then(function () {
        return res.json({'msg': 'Payout toggled'});
    });
});

secureRoutes.post('/toggleTlg', function (req, res) {
    global.mysql.query("UPDATE users SET enable_tlg = NOT enable_tlg WHERE id = ?", [req.decoded.id]).then(function () {
        return res.json({'msg': 'Telegram toggled'});
    });
});

secureRoutes.post('/toggleTlgPayout', function (req, res) {
    global.mysql.query("UPDATE users SET enable_tlg_payout = NOT enable_tlg_payout WHERE id = ?", [req.decoded.id]).then(function () {
        return res.json({'msg': 'Telegram Payout toggled'});
    });
});
