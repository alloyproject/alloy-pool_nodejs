...
function updateActiveConfig() {
    global.mysql.query("SELECT * FROM config WHERE module = 'pool'").then(function (rows) {
        ...
        rows.forEach(function (row) {
            if (row.item == "targetTime") {
                global.config.pool.targetTime = parseInt(row.item_value);
            }
            if (row.item == "minerTimeout") {
                global.config.pool.minerTimeout = parseInt(row.item_value);
            }
            if (row.item == "minDifficulty") {
                global.config.pool.minDifficulty = parseInt(row.item_value);
            }
            if (row.item == "maxDifficulty") {
                global.config.pool.maxDifficulty = parseInt(row.item_value);
            }
            if (row.item == "buffer") {
                global.config.pool.buffer = parseInt(row.item_value);
            }
       });
    });
}
...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent) {
    ...
    this.updateDifficultyOld = function () {
        ...
        if (this.setNewDiff(newDiff)) this.sendNewJob();
    ...
    if (protoVersion === 1) {
        ...
        this.getJob = function () {
            ...
            if (!this.proxy) {
                ...
                this.cachedJob = {
                    blob: blob,
                    algo: global.coinFuncs.algoTypeStr(activeBlockTemplate.port),
                    variant: global.coinFuncs.variantValue(activeBlockTemplate.port),
                    job_id: newJob.id,
                    target: target,
                    id: this.id
                };
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate) {
    ...
    if (job.difficulty >= global.config.pool.buffer || blockCandidate) {
        ...
        if (height !== db_job_height || difficulty !== activeBlockTemplate.difficulty || time_now - worker.time > 60*1000 || acc >= global.config.pool.buffer) {
            if (acc != 0) {
...
function processShare(miner, job, blockTemplate, params) {
    ...
    if (global.config.pool.trustedMiners && miner.difficulty < 400000 && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 &&
        ...
        if (miner.payout in minerWallets && ++minerWallets[miner.payout].last_ver_shares >= MAX_VER_SHARES_PER_SEC * VER_SHARES_PERIOD) {
            ...
            addProxyMiner(miner);
            if (miner.setNewDiff(miner.calcNewDiff())) miner.sendNewJob();
            return null;
        }
