        ...
        try{
            ...
            global.coinFuncs.getBlockHeaderByHash(blockDataDecoded.hash, function(err, header){
                ...
                if (global.config.general.discordBlockAnnounce === true) {
                    ...
                    var blockEffort = Math.round(blockDataDecodedForBot.shares / header.difficulty * 100);
                    global.support.poolBotMsg("BLOCK " + blockId + " MINED! Reward: " + global.support.coinToDecimal(header.reward) + " " + global.config.general.coinCode + ", Effort: " + blockEffort + "%");
                }
                let txn = global.database.env.beginTxn();
