            ...
            async.doUntil(function (callback_until) {
                ...
                globalMinerList.forEach(function (miner) {
                    if (minerList.indexOf(miner) === -1) {
                        ...
                        // Keep updating PPLNS Shares for miners with no workers until their shares extend past PPLNS depth
                        if (minerStats.pplnsShares !== 0) {
                            minerStats.pplnsShares = localStats.minersPplns[miner];
                            cache_updates[miner] = minerStats;
                        }
                    }
                });
