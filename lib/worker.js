...
function updateWalletStats() {
    async.waterfall([
		...
		console.log(wallet);
		    ...
		    balance: wallet.available_balance + parseInt(wallet.locked_amount),
                    unlocked: parseInt(wallet.available_balance),
                    ts: Date.now()
                });
            });
        },
        function (state, callback) {
            global.coinFuncs.getHeight(function (err, height) {
                if (err !== null){
