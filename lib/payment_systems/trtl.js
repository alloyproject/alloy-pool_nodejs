...
function full_stop(err) {
    ...
    global.support.sendtoTelegram(global.config.admin_telegram, "Payment daemon unable to make payment");
}
...
let shapeshiftQueue = async.queue(function (task, callback) {
    ...
    async.waterfall([
        ...
        function (marketInfo, shapeshiftTxnData, body, intCallback) {
            // body.tx_hash = TRTL transaction hash.
...
let xmrToQueue = async.queue(function (task, callback) {
    ...
    async.waterfall([
        function (intCallback) {
            // Verify if TRTL.to is ready to get to work.
            xmrAPIClient.get('order_parameter_query/', function (err, res, body) {
                if (err) {
                    ...
                    console.log("Attempting to pay: " + address + " Amount: " + amtOfBTC + " BTC or " + amount / global.config.general.sigDivisor + " TRTL");
                    console.log("Response from TRTL.to: " + JSON.stringify(body));
                    if (body.lower_limit >= amtOfBTC) {
                        return intCallback("Not enough TRTL to hit the minimum deposit");
                    } else if (body.upper_limit <= amtOfBTC) {
                        return intCallback("Too much TRTL to pay out to xmr.to");
                    } else {
...
let paymentQueue = async.queue(function (paymentDetails, callback) {
    ...
    console.log("Trying to make payment based on: " + JSON.stringify(paymentDetails));

    function getbalance() {
        global.support.rpcWallet("getBalance", {}, function (body) {
            if (body.hasOwnProperty('error') || !body.hasOwnProperty('result') || typeof(body.result) === 'undefined' || !body.result.hasOwnProperty('availableBalance') || typeof(body.result.availableBalance) !== "number") {
                console.error("Can't getBalance: " + JSON.stringify(body));
                setTimeout(getbalance, 60*1000);
                return;
            }
            if (body.result.availableBalance === 0) {
                console.log("Waiting for balance to unlock after previous payment");
                setTimeout(getbalance, 5*60*1000);
                return;
            }
            console.log("Current wallet balance is " + global.support.coinToDecimal(body.result.lockedAmount) + " with " + global.support.coinToDecimal(body.result.availableBalance) + " unlocked balance");
            ...
            global.support.rpcWallet(transferFunc, paymentDetails, function (body) {
                console.log("Payment made: " + JSON.stringify(body));
                if (body.hasOwnProperty('error') || !body.hasOwnProperty('result')) {
                    if (typeof(body.error) !== 'undefined' && body.error.hasOwnProperty('message') && body.error.message === "not enough money"){
...
function Payee(amount, address, paymentID, bitcoin) {
    ...
    this.makePaymentWithID = function () {
		let paymentDetails = {
			...
			addresses : [global.config.pool.address],
        ...
        debug("Payment Details: " + JSON.stringify(paymentDetails));
        paymentQueue.push(paymentDetails, function (body) {
            if (typeof body.transactionHash !== 'undefined') {
                console.log("[*] Successful payment to " + identifier + " of " + global.support.coinToDecimal(amount) + " TRTL (fee " + global.support.coinToDecimal(fee) + " - " + global.support.coinToDecimal(global.config.payout.fee) + " = " + global.support.coinToDecimal(fee - global.config.payout.fee) + ") with tx_hash " + body.transactionHash.match(hexChars)[0]);
                global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                    [0, address, paymentID, amount, body.transactionHash.match(hexChars)[0], global.config.payout.mixIn, global.config.payout.fee, 1]).then(function (result) {
                    if (!result.hasOwnProperty("affectedRows") || result.affectedRows != 1) {
                        console.error("Can't do: INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (0, '"
                            + address + "', '" + paymentID + "', " + amount + ", '" + body.transactionHash.match(hexChars)[0] + "', " + global.config.payout.mixIn + ", " + global.config.payout.fee + ", 1)"
                        );
                    ...
                    payee.tx_hash = body.transactionHash.match(hexChars)[0];
    ...
    this.trackPayment = function () {
        ...
        global.mysql.query("SELECT email FROM users WHERE username = ? AND enable_payout IS true limit 1", [payee.id]).then(function (rows) {
            ...
            global.support.sendEmail(rows[0].email,
                sprintf("Your %(payment_amount)s TRTL payment was just performed", emailData),
                sprintf(
                    "Your payment of %(payment_amount)s TRTL (with tx fee %(fee)s TRTL) to %(address)s wallet was just performed and total due was decreased by %(amount)s TRTL.\n" +
        ...
        global.mysql.query("SELECT telegram_id FROM users WHERE username = ? AND enable_tlg_payout IS true limit 1", [payee.id]).then(function (rows) {
            if (rows.length === 0 || rows[0].telegram_id === null) return;
            if (rows[0].telegram_id.length < 5) console.error("don't enter correct TLG ID of " + payee.id);return;
            // toAddress, subject, body
            let emailData = {
                address: payee.id,
                payment_amount: global.support.coinToDecimal(payee.amount - payee.fee),
    ...
    global.mysql.query("SELECT * FROM balance WHERE amount >= ?", [global.support.decimalToCoin(global.config.payout.walletMin)]).then(function (rows) {
        ...
        rows.forEach(function (row) {
            ...
            global.mysql.query("SELECT payout_threshold FROM users WHERE username = ?", [payee.id]).then(function (userRow) {
                roundCount += 1;
                let threshold = global.support.decimalToCoin(100);
                ...
                if (roundCount === rows.length && paymentDestinations.length > 0) {
                    while (paymentDestinations.length > 0) {
                        ...
                        console.log("Adding payment for " + paymentDetails.transfers.length + " miners");
                        paymentQueue.unshift(paymentDetails, function (body) {  //jshint ignore:line
                            ...
                            if (typeof body.transactionHash !== 'undefined') {
                                ...
                                paymentDetails.transfers.forEach(function (payeeItem) {
                                    ...
                                    console.log("[**] Successful payment to " + payeeItem.address + " for " + global.support.coinToDecimal(payeeObjects[payeeItem.address].amount) + " TRTL (fee " + global.support.coinToDecimal(payeeObjects[payeeItem.address].fee) + ")");
                                });
                                console.log("[*] Successful payment to multiple miners of " + global.support.coinToDecimal(totalAmount) + " TRTL (fee " + global.support.coinToDecimal(totalFee) + " - " + global.support.coinToDecimal(global.config.payout.fee) + " = " + global.support.coinToDecimal(totalFee) + ") with tx_hash " + body.transactionHash.match(hexChars)[0]);
                                global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                                    [0, null, null, totalAmount, body.transactionHash.match(hexChars)[0], global.config.payout.mixIn, global.config.payout.fee, paymentDetails.transfers.length]).then(function (result) {
                                    if (!result.hasOwnProperty("affectedRows") || result.affectedRows != 1) {
                                        console.error("Can't do: INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (0, null, null, "
                                            + totalAmount + ", '" + body.transactionHash.match(hexChars)[0] + "', " + global.config.payout.mixIn + ", " + body.fee + ", " + paymentDetails.transfers.length + ")"
                                        );
                                        paymentDetails.transfers.forEach(function (payeeItem) {
