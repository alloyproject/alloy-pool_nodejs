...
let paymentQueue = async.queue(function (paymentDetails, callback) {
	...
	//console.log(paymentDetails.destinations)
		...
		//console.log(body.result.available_balance)
		    ...
		    extraPaymentRound = true;
                    setTimeout(function () {
			console.log('IN EXTRA PAYMENT',extraPaymentRound);
                        makePayments();
                    }, global.config.payout.timerRetry * 1000);
		}
	    ...
	    console.log(body)
            ...
            global.support.sendEmail(global.config.general.adminEmail, "0 Payment daemon unable to check wallet balance",
	...
	if (body.hasOwnProperty('error')) {
		...
		console.log(body)
                ...
                console.error("Will not make more payments until the payment daemon is restarted!");
                //toAddress, subject, body
                global.support.sendEmail(global.config.general.adminEmail, "1 Payment daemon unable to make payment",
			...
			console.log(global.config.payout.mixIn)
			   ...
			   //if (body.fee && body.fee > 10) {
				...
				console.log(0, null, null, totalAmount, body.tx_hash.match(hexChars)[0], global.config.payout.mixIn, 0.1, paymentDetails.destinations.length)
                            ...
                            //} else {
//                                console.error("3 Unknown error from the wallet.");
                            //}
                        });
                    }
                }
