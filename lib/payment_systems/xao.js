...
let paymentQueue = async.queue(function (paymentDetails, callback) {
    ...
    global.support.rpcWallet('getbalance', [], function (body) {
        ...
        if (body.result) {
            ...
            if (body.result.unlocked_balance < amountToPay) {
                console.error("Wallet only has " + body.result.unlocked_balance + " unlocked balance, can't pay " + amountToPay + " worth of XAO. Retrying in " + timerRetry + " minutes!");
    ...
    global.support.rpcWallet(transferFunc, paymentDetails, function (body) {
        ...
        if (paymentDetails.hasOwnProperty('payment_id')) {
            console.log("Payment made to " + paymentDetails.destinations[0].address + " with PaymentID: " + paymentDetails.payment_id + " For: " + global.support.coinToDecimal(paymentDetails.destinations[0].amount) + " XAO with a " + global.support.coinToDecimal(body.result.fee) + " XAO Mining Fee");
            return callback(body.result);
        } else {
            if (transferFunc === 'transfer') {
                console.log("Payment made out to multiple people, total fee: " + global.support.coinToDecimal(body.result.fee) + " XAO");
            }
            let intCount = 0;
