...
let paymentQueue = async.queue(function (paymentDetails, callback) {
    ...
    global.support.rpcWallet('getbalance', [], function (body) {
        console.log(body.result) //we log the balance
        if (body.result) {
            var amountToPay = paymentDetails.destinations.reduce(function (a, b) { return a + b; }, 0);
            if (body.result.unlocked_balance < amountToPay) {
                console.error("Wallet only has " + body.result.unlocked_balance + " unlocked balance, can't pay " + amountToPay + " worth of XLC. Retrying in " + timerRetry + " minutes!");
            ...
            console.log(body.error) //we log errors
    ...
    global.support.rpcWallet(transferFunc, paymentDetails, function (body) {
        ...
        if (paymentDetails.hasOwnProperty('payment_id')) {
            console.log("Payment made to " + paymentDetails.destinations[0].address + " with PaymentID: " + paymentDetails.payment_id + " For: " + global.support.coinToDecimal(paymentDetails.destinations[0].amount) + " XLC with a " + global.support.coinToDecimal(body.result.fee) + " XLC Mining Fee");
            return callback(body.result);
        } else {
            if (transferFunc === 'transfer') {
                console.log("Payment made out to multiple people, total fee: " + global.support.coinToDecimal(body.result.fee) + " XLC");
            }
            let intCount = 0;
            paymentDetails.destinations.forEach(function (details) {
                console.log("Payment made to: " + details.address + " For: " + global.support.coinToDecimal(details.amount) + " XLC");
                intCount += 1;
        ...
        paymentQueue.push(paymentDetails, function (body) {
            if (body.fee && body.fee > 10) {
                ...
                if (global.config.general.discordPayoutAnnounce === true) {
                    global.support.poolBotMsg("Pool paid out: " + global.support.coinToDecimal(amount)  + " XLC to 1 miner");
                }
            } else {
                console.error("1 Unknown error from the wallet.");
            }
        });
    };
                ...
                if (roundCount === rows.length && paymentDestinations.length > 0) {
                    while (paymentDestinations.length > 0) {
                        ...
                        paymentQueue.push(paymentDetails, function (body) {  //jshint ignore:line
                           ...
                           console.log("got so far") //just logging for debug purposes
                           console.log(body) //more logging
                                ...
                                console.log(body.tx_hash.match(hexChars)[0]) //we need some logs to see various variables returned from transactions
                                console.log(global.config.payout.mixIn) //more logging
                                ...
                                if (global.config.general.discordPayoutAnnounce === true) {
                                    global.support.poolBotMsg("Pool paid out: " + global.support.coinToDecimal(totalAmount)  + " XLC to " + paymentDetails.destinations.length + " miners");
                                }
