...
function Coin(data){
    ...
    this.cryptoNight = function(convertedBlob) {
        return multiHashing.cryptonight_heavy(convertedBlob);
    }

    this.blobTypeStr = function(port) {
        switch (port) {
            case 48782: return "forknote2"; // ITNS
    ...
    this.get_miner_agent_notification = function(agent) {
        ...
        if (m = reXMRig.exec(agent)) {
            ...
            if (majorv + minorv < 205) {
                return "Please update your XMRig miner (" + agent + ") to v2.5.0+ (and use cryptonight-heavy in config";
            }
        } else if (m = reXMRSTAK.exec(agent)) {
            let majorv = parseInt(m[1]) * 100;
            let minorv = parseInt(m[2]);
            if (majorv + minorv < 203) {
                return "Please update your xmr-stak miner (" + agent + ") to v2.4.3+ (and use cryptonight_heavy in config)";
            }
    ...
    this.get_miner_agent_warning_notification = function(agent) {
        ...
        if (m = reXMRSTAK2.exec(agent)) {
            ...
            let minorv = parseInt(m[2]) * 100;
            let minorv2 = parseInt(m[3]);
            if (majorv + minorv + minorv2 < 102) {
                 return "Please update your xmr-node-proxy (" + agent + ") to version v0.1.2+ (check https://github.com/MoneroOcean/xmr-node-proxy repo)";
            }
        }
        return false;
    };
